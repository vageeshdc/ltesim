...
static void MultiUlDlbkp (int nbCell, double radius,
                       int nbUE,
                       int nbVoIP, int nbVideo, int nbBE, int nbCBR,
                       int sched_typeUL,int sched_typeDL,
                       int frame_struct,
                       int speed,
                       ...
                       int seed)
{
    ...
    int nbBe_ = nbBE;
    int tot_pkt =0,maxPkt;
    /*
    if(nbBE == 0){
      std::cout<<"Zero base - rectify it\n";
      nbBE = 1;
      maxPkt = 1;
    }
    else{
      */
      maxPkt = 1+(nbBE*nbCell*nbUE);
    //}

    //Define Application Container
    VoIP VoIPApplication[nbVoIP*nbCell*nbUE];
    TraceBased VideoApplication[nbVideo*nbCell*nbUE];
    InfiniteBuffer BEApplication[maxPkt];
    ...
    for (int j = 0; j < nbCell; j++)
    {
        ...
        for (int i = 0; i < nbUE; i++)
        {
			  ...
			  if(j_%2 == 0){
                    ...
                    if (downlink_scheduler_type == ENodeB::DLScheduler_TYPE_FLS)
                {
                  ...
                  if (maxDelay == 0.1)
                    ...
                    //create classifier parameters
                    ClassifierParameters *cp = new ClassifierParameters (gw->GetIDNetworkNode(),ue->GetIDNetworkNode(),
            ...
            // *** be application
            for (int j_ = 0; (j_ < nbBE)&&(tot_pkt < maxPkt); j_++)
            {
		tot_pkt++;
		if(j_%2 == 0){
                // create application
		...
		//if(nbBe_ == 0){
		//  BEApplication[beApplication].SetStopTime(start_time - 0.1);
		//}
		//else{
