...
static void UplinkChannelQuality ()
{
...
Gateway* gw = new Gateway();
  ...
  UserEquipment* ue[nbUEs];
  Application* be[nbUEs];
  ...
  for (int i = 0; i < nbUEs; i++)
    {
	  ...
	  networkManager->GetUserEquipmentContainer ()->push_back (ue[i]);
	  ue[i]->SetTargetNode (enb);
	  enb->RegisterUserEquipment (ue[i]);

	  std::cout << "Created UE - id " << idUe << " position " << posX << " " << posY << " direction " << speedDirection << std::endl;

	  ue[i]->GetPhy ()->SetDlChannel (enb->GetPhy ()->GetDlChannel ());
	  ue[i]->GetPhy ()->SetUlChannel (enb->GetPhy ()->GetUlChannel ());

      FullbandCqiManager *cqiManager = new FullbandCqiManager ();
      cqiManager->SetCqiReportingMode (CqiManager::PERIODIC);
      cqiManager->SetReportingInterval (1);
      ue[i]->SetCqiManager (cqiManager);
      cqiManager->SetDevice (ue[i]);

      WidebandCqiEesmErrorModel *errorModel = new WidebandCqiEesmErrorModel ();
      ue[i]->GetPhy ()->SetErrorModel (errorModel);

	  MacroCellUrbanAreaChannelRealization* c_dl = new MacroCellUrbanAreaChannelRealization (enb, ue[i]);
	  enb->GetPhy ()->GetDlChannel ()->GetPropagationLossModel ()->AddChannelRealization (c_dl);
	  MacroCellUrbanAreaChannelRealization* c_ul = new MacroCellUrbanAreaChannelRealization (ue[i], enb);
	  enb->GetPhy ()->GetUlChannel ()->GetPropagationLossModel ()->AddChannelRealization (c_ul);

	  ue[i]->GetPhy ()->GetDlChannel ()->AddDevice (ue[i]);
	 ...
	 if((i % 2) == 0){ 
	  be[i] = flowsManager->CreateApplication (applicationID,
		                                   			     ue[i], enb,
