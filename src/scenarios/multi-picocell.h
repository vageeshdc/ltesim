...
static void SingleCellWithPico ( double radius,int nbFemtoCells,
                       ...
                       int seed)
{
  ...
  for (int i = 0; i < nbCell; i++)
    ...
    // creating pico cells
    for(int i_ = 0+nbCell;i_< nbFemtoCells+nbCell;i_++){
          Cell *c = new Cell (i_, radius/2, 0.035, (rand()%((int)ceil(radius*1000)))/2000,(rand()%((int)ceil(radius*1000)))/2000);
	  cells->push_back (c);
	  nm->GetCellContainer ()->push_back (c);
    }
 ...
 // std::vector <BandwidthManager*> femto_spectrums = RunFrequencyReuseTechniques (nbCell, cluster, bandwidth);
  ...
  //create Home eNBs
  std::vector <Cell*> *piccells = nm->GetCellContainer();
  for (int i = nbCell; i < nbCell + nbFemtoCells; i++)
  {
		  ENodeB* enb = new ENodeB (i, piccells->at (i),1,23);
		  enb->GetPhy ()->SetDlChannel (dlChannels->at (i));
		  enb->GetPhy ()->SetUlChannel (ulChannels->at (i));
  ...
  //nbUE is the number of users that are into each cell at the beginning of the simulation
  //idUE = nbCell*nbUE;
  std::vector <ENodeB*> *HeNBs = nm->GetENodeBContainer();
  for (int j = 0+nbCell; j < nbFemtoCells+nbCell; j++)
    {

      int idCell = j;
      vector<CartesianCoordinates*> *positions = GetUniformUsersDistribution (idCell, nbFemtoUE);
...
*/ 
		  ...
		  // define the channel realization
		  MacroCellUrbanAreaChannelRealization* c_dl = new MacroCellUrbanAreaChannelRealization (HeNBs->at (j), ue);
		  c_dl->SetChannelType (ChannelRealization::CHANNEL_TYPE_PED_A);
		  ...
		  MacroCellUrbanAreaChannelRealization* c_ul = new MacroCellUrbanAreaChannelRealization (ue,HeNBs->at (j));
